...
class CRUDUser(CRUDBase[User, UserCreate, UserUpdate]):
    ...
    def authenticate(self, db: Session, *, email: str, password: str) -> Optional[User]:
        ...
        if not user:
            return None
        valid, new_hashed_password = verify_and_update_password(password, user.hashed_password)
        if not valid:
            return None
        if new_hashed_password:
            self.update(db, db_obj=user, obj_in={"hashed_password": new_hashed_password})
        return user
